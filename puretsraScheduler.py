...
from pureTimeSensitiveResourceAllocator import PureTimeSensitiveResourceAllocator
from scheduler import Scheduler

class PureTSRAScheduler(Scheduler):
	...
	def runScheduler(self, avgSystemLoad):
		...
		totalWelfare = 0
		totalWinningUser = 0
		pureallocator = PureTimeSensitiveResourceAllocator()
		...
		for timeStamp in xrange(0, constants.MAX_TIME_INTERVAL + 1):
			...
			if len(bundles) > 0:
				welfare, winnerLoser = pureallocator.simulate(self.resourceCaps, bundles, self.vmResourceRequirement, None, bids, deadlines)
		...
		if constants.CLEAR_THE_POOL is True:
			totalWelfare, totalWinningUser = self.clearWaitingPool(pureallocator, avgSystemLoad, totalWelfare, totalWinningUser)
